	...
	public void onUWAPIRequestComplete(JSONObject jsonObject, boolean success) {
		...
		try {
			...
			for (int i = 0; i < jsonData.length(); i++) {
				switch (i) {
				case 0:
					jsonOMGUW = jsonData.getJSONObject(OMG);
					break;

				case 1:
					jsonOMGUW = jsonData.getJSONObject(ILU);
					break;

				case 2:
					jsonOMGUW = jsonData.getJSONObject(MCs);
					break;

				case 3:
					jsonOMGUW = jsonData.getJSONObject(OVERHEARDs);
					break;

				case 4:
					jsonOMGUW = jsonData.getJSONObject(ASK);
					break;
				...
				for (int j = 0; j < jsonOMGResult.length(); j++) {
					...
					omguws.add(
							j,
							new OMGUWData(omguwObject.getString("ID"),
									omguwObject.getString("Date"), omguwObject
											.getString("Content"), omguwObject
											.getString("Link"), omguwObject
											.getString("Type")));
				}
				omguwsList.add(i, omguws);

				OMGUWListadapters.add(i,
						new OMGUWListAdapter(ctx, omguwsList.get(i)));
			}
