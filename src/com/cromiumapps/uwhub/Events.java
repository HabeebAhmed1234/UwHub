	...
	try {
		if(this.CURRENT_QUERY == 0)
		{
			...
			for (int j = 0; j < jsonEvents.length(); j++) {
			    ...
			    tempEventsData.add(j, new EventsData(EventsObject.getString("ID"),date, EventsObject.getString("Name"), EventsObject.getString("Links"), EventsObject.getString("Description"),""));
		...
		if(this.CURRENT_QUERY == 1)
		{
			...
			for (int j = 0; j < jsonEvents.length(); j++) {
			    JSONObject EventsObject = jsonEvents.getJSONObject(j);
			    tempEventsData.add(j, new EventsData(EventsObject.getString("ID"), EventsObject.getString("When"), EventsObject.getString("Title"), EventsObject.getString("Link"), EventsObject.getString("Description"),EventsObject.getString("Where")));
		...
		if(this.CURRENT_QUERY == 2)
		{
			...
			for (int j = 0; j < jsonHolidays.length(); j++) {
			    ...
			    tempEventsData.add(j, new EventsData(Integer.toString(j), tempHoliday.getString("Day"), tempHoliday.getString("Name"), "", "",""));
				//tempEventsData.add(j, new EventsData("0", "2013-19-9", "Name", "Links", "Description"));
			}
